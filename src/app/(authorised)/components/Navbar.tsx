...
export default function NavBar() {
	...
	useEffect(() => {
		if (id) {
			const getUser = async () => {
				const userDetails = await getUserDetails(String(id));
		...
		<nav className="w-full flex justify-between items-center h-20 bg-white text-black px-5">
			<div className="flex flex-col gap-1 h-14 px-4">
				<div className="flex gap-2 items-center">
					{urlParts[1]?.includes("task") && (
						<button
							onClick={goBack}
							style={{ fontSize: "0.85rem" }}
							className="py-1 px-2 bg-primary cursor-pointer text-white rounded-full"
						>
							&larr;
						</button>
					)}
					<div className="">
						{urlParts[1]?.startsWith("dashboard") ? (
